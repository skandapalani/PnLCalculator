...
TEST(PnLCalculatorEdgeCases, SellBeforeBuy) {
    PnLCalculator calc(true);
    std::vector<Event> events = {
        makeEvent(1, "AAPL", 'S', 110, 10)
    };
    testing::internal::CaptureStdout();
    calc.processTrades(events);
    std::string output = testing::internal::GetCapturedStdout();
...
TEST(PnLCalculatorEdgeCases, ZeroQuantityTrade) {
    PnLCalculator calc(true);
    std::vector<Event> events = {
...
TEST(PnLCalculatorEdgeCases, MultipleSymbolsInterleaved) {
    PnLCalculator calc(true);
    std::vector<Event> events = {
        makeEvent(1, "AAPL", 'B', 100, 10),
        makeEvent(2, "GOOG", 'B', 200, 5),
